...
def train_model(model, train_data, dev_data, device, num_epochs=5, learning_rate=1e-5, early_stopping_patience=3):
    ...
    for epoch_i in range(num_epochs):
        ...
        val_loss_arr.append(avg_dev_loss)
    ...
    print("\nTraining complete!")
    return model,sum(train_loss_arr) / len(train_loss_arr),sum(val_loss_arr) / len(val_loss_arr)
def train_bins_ensemble(
    ...
    for bi, bin_df in enumerate(bins, 1):
        ...
        model,avgtrainloss,avgvalloss = train_model(
            model=model,
        ...
        train_loss_arra.append(avgtrainloss)
        dev_loss_arr.append(avgvalloss)
    ...
    return dev_acc, dev_mcc, dev_probs_ens, test_probs_ens,train_loss_arra,dev_loss_arr
...
def finetune_paraphrase_detection(args):
    ...
    if args.k_bins and args.k_bins > 0:
        ...
        dev_acc, dev_mcc, dev_probs_ens, test_probs_ens,train_loss_arr,dev_loss_arr = train_bins_ensemble(
        ...
        metrics = {
            ...
            "val_mcc": dev_mcc,
            'train_loss_array':train_loss_arr,
            'val_loss_array':dev_loss_arr
        }
